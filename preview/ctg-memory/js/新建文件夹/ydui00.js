...
//		_this.loadCountry();
        ...
        if (defaultSet.country) {
            ...
            $cityElement.find('.cityselect-nav a').eq(1).addClass('crt').html('请选择');
        }
        ...
        if (defaultSet.city) {
            ...
            _this.setNavTxt(1, defaultSet.city, defaultSet.id.split('-')[1]);
        }

        if (defaultSet.area) {
            _this.loadArea();
            _this.ForwardViewTwo(true);
            _this.setNavTxt(2, defaultSet.area, defaultSet.id.split('-')[2]);
        }
...
//            '        </li>' +            
        ...
        _this.$cityElement.on('click.ydui.cityselect', '.cityselect-nav a', function () {
            ...
            $this.index() < 1 ? _this.backOffView() : $this.index() == 1 ? _this.ForwardView(true) : _this.ForwardViewTwo(true);
        });
    };
    ...
    CitySelect.prototype.bindItemEvent = function () {
        ...
        $cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function () {
            ...
            var $nav = $cityElement.find('.cityselect-nav a'),
             ...
             if (tag == 0) {
                _this.loadProvance();
                $cityElement.find('.cityselect-item-box').eq(0).find('a').removeClass('crt');
...
//            arr.push($('<a cid="1" class="' + '" href="javascript:;"><span>' + v.CountryName + '</span></a>').data({
//                citys: v.Province,
//                tag: 0
//            }));
//        });
